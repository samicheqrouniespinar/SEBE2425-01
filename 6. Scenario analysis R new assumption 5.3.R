...
# Date - 09/08/2025
# Description of new script: Implementing assumption 5.3
...
# New assumption 5.3: Internal temperature is greater than 20 degreesC 
...
filtered_data_new5.2 <- filtered_data %>%
  filter(On_Off_flag == "On" & Hour_Minute >= "17:00:00" & run_length_mins >= 60 | 
           On_Off_flag == "On - Compressor Off" & Hour_Minute >= "17:00:00" | 
           On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 20 & External_temp_range <= 2) 
...
filtered_data_on_new5.2 <- filtered_data %>%
...
filtered_data_off_new5.2 <- filtered_data %>%
  filter(On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 20 & External_temp_range <= 2)

# Filtering for when the dates in the 'On' an 'Off' periods match 
filtered_data_on_new5.2 <- filter(filtered_data_on_new5.2, date %in% filtered_data_off_new5.2$date & Property_ID %in% filtered_data_off_new5.2$Property_ID)
...
filtered_data_new5.2 <- left_join(filtered_data_new5.2,EoH_meta_data, by = "Property_ID")

# Ensuring HP size is formatted as a number 
filtered_data_new5.2$HP_Size_kW <- as.numeric(filtered_data_new5.2$HP_Size_kW)

# Calculating the average hourly consumption across a given period of on or off
filtered_data_new5.2 <- filtered_data_new5.2 %>%
...
filtered_data_new5.2$percent_work_output <- round(filtered_data_new5.2$percent_work_output, digits = 1)
...
filtered_data_new5.2 <- unique(filtered_data_new5.2) 
...
filtered_data_new5.2$date <- ymd(filtered_data_new5.2$date)

# Ensure the above variable is formatted as a number 
filtered_data_new5.2$On_Off_flag_num <- as.numeric(filtered_data_new5.2$On_Off_flag_num)
...
filtered_data_new5.2 <- na.omit(filtered_data_new5.2)
...
#filtered_data_new5.2$consecutive <- filtered_data_new5.2$consecutive - lag(filtered_data_new5.2$consecutive, default = 0)
...
filtered_data_new5.2$Technical_potential_demand_response_kW <- round(filtered_data_new5.2$Technical_potential_demand_response_kW, digits = 2)
filtered_data_new5.2$Shiftable_energy_potential_kWh <- round(filtered_data_new5.2$Shiftable_energy_potential_kWh, digits = 2)
...
filtered_data_new5.2$hourly_flexibility_potential_kWh <- round(filtered_data_new5.2$hourly_flexibility_potential_kWh, digits = 2)
filtered_data_new5.2$half_hourly_flexibility_potential_kWh <- round(filtered_data_new5.2$half_hourly_flexibility_potential_kWh, digits = 2)
...
filtered_data_new5.2$hourly_flexibility_potential_kWh <- as.numeric(filtered_data_new5.2$hourly_flexibility_potential_kWh)
filtered_data_new5.2$total_hourly_consumption_kWh <- as.numeric(filtered_data_new5.2$total_hourly_consumption_kWh)
filtered_data_new5.2$total_hourly_consumption_kWh_lag <- as.numeric(filtered_data_new5.2$total_hourly_consumption_kWh_lag)

table(filtered_data_new5.2$date)
...
filtered_data_new5.2$error_rate <- round(filtered_data_new5.2$error_rate, digits = 1)
filtered_data_new5.2$accuracy <- round(filtered_data_new5.2$accuracy, digits = 1)
...
#post_processing_full <- filtered_data_new5.2 %>%
...
post_processing_full_new5.2 <- filtered_data_new5.2 %>%
  filter(consecutive_new == 1 | consecutive_new == -1)

# Creating a dataframe for flexibility periods within the dataset
flexibility_periods_new5.2 <- post_processing_full_new5.2 %>%
  filter(On_Off_flag == "Off")

# Converting run_id into a factor to be able to plot  
flexibility_periods_new5.2$run_id <- as.factor(flexibility_periods_new5.2$run_id)

# Filtering for just Off periods to be able to look at accuracy 
flexibility_periods_full_new5.2 <- flexibility_periods_new5.2 %>%
  filter(On_Off_flag == "Off")

# Technical potential for demand response in each period 
Plot_106 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Technical_potential_demand_response_kW)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW), colour = "red", linetype = 'dotted', size = 1) +
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) + 0.3, label = "Mean technical potential for demand response", colour = "red", size = 3) +
  labs(x = "'Off' period" ,
...
Plot_106 <- Plot_106 + guides(fill=guide_legend(title="Property ID")) 
Plot_106

# Move accuracy and error columns so that 
flexibility_periods_full_new5.2 <- flexibility_periods_full_new5.2  %>%  
  relocate(accuracy, .before = Interval_consumption_kWh) %>%
  relocate(error_rate, .before = Interval_consumption_kWh)

# Getting rid of Null values
flexibility_periods_full_new5.2 <- na.omit(flexibility_periods_full_new5.2)
...
median(flexibility_periods_full_new5.2$Measured_accuracy_percent) # median accuracy of results i 98.3%
mean(flexibility_periods_full_new5.2$Measured_accuracy_percent) # mean accuract is 95%
summary(flexibility_periods_full_new5.2$Measured_accuracy_percent)

str(post_processing_full_new5.2)

# Converting run ID into factor to be able to plot 
post_processing_full_new5.2$run_id <- as.factor(post_processing_full_new5.2$run_id)
flexibility_periods_full_new5.2$run_id <- as.factor(flexibility_periods_full_new5.2$run_id)

# Visualising the accuracy
Plot_107 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Measured_accuracy_percent)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.2$Measured_accuracy_percent), colour = "red", linetype = 'dotted', size = 1) +
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.2$Measured_accuracy_percent) + 6, label = "Average accuracy of calculated period", colour = "red", size = 3) +
  labs(x = "run ID" ,
       y = "Accuracy (%) ",
...
mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) # 2.06kW  per period
median(flexibility_periods_full_new5.2$half_hourly_flexibility_potential_kWh) # 0.97 kW per half hour of flexibility
summary(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh) # 5.958 kWh of technical potential per event  
sd(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh) # 3.79 (very large)

# Summary statistics of results 
summary(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh)
sd(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh)

# Calculating optimal number of bins for plotting histogram

# Finding value of n for size of data
n1 <- n_distinct(flexibility_periods_full_new5.2)
print(n1)  #610 data size

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) - min(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW)

Q3 <- quantile(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW, 0.25) 
...
Plot_108 <- ggplot(flexibility_periods_full_new5.2, aes(x = Technical_potential_demand_response_kW)) +
  geom_histogram(bins = Freedman_Diaconic_bin3, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW), colour = "orange", linetype = 'dotted', size = 1)+
  #annotate("label", x =  median(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 60, label = "Median demand response per period (kW)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW), colour = "red", linetype = 'dotted', size = 1)+
  #annotate("label", x =  mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 50, label = "Mean demand response per period (kW)", colour = "red", size = 3) +
...
Plot_108 <- Plot_108 + guides(fill=guide_legend(title="Property ID")) 
Plot_108

# Boxplot of technical potential for demand response
Plot_109 <- ggplot(flexibility_periods_full_new5.2, aes(y = Technical_potential_demand_response_kW)) +
...
Plot_109 <- Plot_109 + guides(fill=guide_legend(title="Property ID")) 
Plot_109

Panel_plot1 <- Plot_108 + Plot_109
Panel_plot1

# Length of flexibility periods 
summary(flexibility_periods_full_new5.2$Heat_free_hours) # 1.167 hours of flexibility per heat pump
sd((flexibility_periods_full_new5.2$Heat_free_hours))

# Bar graph of seeing the average run length of each flexibility period
Plot_110 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = (Heat_free_hours))) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.2$Heat_free_hours), colour = "red", linetype = 'dotted', size = 1)+
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.2$Heat_free_hours) + .7 , label = "Average length of period", colour = "red", size = 3) +
  labs(x = "Period" ,
...
Plot_110 <- Plot_110 + guides(fill=guide_legend(title="Property ID")) 
Plot_110

# Bar graph of seeing the saved energy of each period with demand response potential
Plot_111 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Shiftable_energy_potential_kWh)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh), colour = "red", linetype = 'dotted', size = 1)+
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.2$Shiftable_energy_potential_kWh) + .7 , label = "Average saved energy per period", colour = "red", size = 3) +
  labs(x = "Period" ,
...
Plot_111 <- Plot_111 + guides(fill=guide_legend(title="Property ID")) 
Plot_111

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.2$Heat_free_hours)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.2$Heat_free_hours) - min(flexibility_periods_full_new5.2$Heat_free_hours)

Q3 <- quantile(flexibility_periods_full_new5.2$Heat_free_hours, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.2$Heat_free_hours, 0.25) 
...
# Histogram of number of heat free horus 
Plot_112 <- ggplot(flexibility_periods_full_new5.2, aes(x = (Heat_free_hours))) +
  geom_histogram(bins = Freedman_Diaconic_bin4, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.2$Heat_free_hours), colour = "orange", linetype = 'dotted', size = 1) +
  #annotate("label", x =  median(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 60, label = "Median length of time (hours)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.2$Heat_free_hours), colour = "red", linetype = 'dotted', size = 1) +
  #annotate("label", x =  mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 50, label = "Mean length of time (hours)", colour = "red", size = 3) +
...
Plot_101 <- Plot_101 + guides(fill=guide_legend(title="Property ID")) 
Plot_101

# Summary of internal temperature decay 
summary(flexibility_periods_full_new5.2$Internal_temperature_decay)
sd(flexibility_periods_full_new5.2$Internal_temperature_decay)

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.2$Internal_temperature_decay)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.2$Internal_temperature_decay) - min(flexibility_periods_full_new5.2$Internal_temperature_decay)

Q3 <- quantile(flexibility_periods_full_new5.2$Internal_temperature_decay, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.2$Internal_temperature_decay, 0.25) 
...
Plot_113 <- ggplot(flexibility_periods_full_new5.2, aes(x = (Internal_temperature_decay))) +
  geom_histogram(bins = Freedman_Diaconic_bin5, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.2$Internal_temperature_decay), colour = "orange", linetype = 'dotted', size = 1)+
  #annotate("label", x =  median(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 60, label = "Median length of time (hours)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.2$Internal_temperature_decay), colour = "red", linetype = 'dotted', size = 1)+
  #annotate("label", x =  mean(flexibility_periods_full_new5.2$Technical_potential_demand_response_kW) , y = 50, label = "Mean length of time (hours)", colour = "red", size = 3) +
...
Plot_113 <- Plot_113 + guides(fill=guide_legend(title="Property ID")) 
Plot_113
