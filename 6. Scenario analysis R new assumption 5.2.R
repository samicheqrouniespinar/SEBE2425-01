...
# Date - 09/08/2025
# Description of new script: Implementing assumption 5.2
...
# New assumption 5.2: Internal temperature is greater than 19 degreesC 
...
filtered_data_new5.2 <- filtered_data %>%
  filter(On_Off_flag == "On" & Hour_Minute >= "17:00:00" & run_length_mins >= 60 | 
           ...
           On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 19 & External_temp_range <= 2) 
...
filtered_data_off_new5.2 <- filtered_data %>%
  filter(On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 19 & External_temp_range <= 2)
...
Plot_97 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Technical_potential_demand_response_kW)) +
...
Plot_97 <- Plot_97 + guides(fill=guide_legend(title="Property ID")) 
Plot_97
...
Plot_98 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Measured_accuracy_percent)) +
  geom_bar(stat = "identity") +
...
Plot_99 <- ggplot(flexibility_periods_full_new5.2, aes(x = Technical_potential_demand_response_kW)) +
...
Plot_99 <- Plot_99 + guides(fill=guide_legend(title="Property ID")) 
Plot_99

# Boxplot of technical potential for demand response
Plot_100 <- ggplot(flexibility_periods_full_new5.2, aes(y = Technical_potential_demand_response_kW)) +
...
Plot_100 <- Plot_100 + guides(fill=guide_legend(title="Property ID")) 
Plot_100

Panel_plot1 <- Plot_99 + Plot_100
Panel_plot1

# Length of flexibility periods 
summary(flexibility_periods_full_new5.2$Period_length_minutes)/60 # 1.167 hours of flexibility per heat pump
sd((flexibility_periods_full_new5.2$Period_length_minutes)/60)

# Bar graph of seeing the average run length of each flexibility period
Plot_101 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = (Heat_free_hours))) +
  geom_bar(stat = "identity") +
...
Plot_102 <- Plot_102 + guides(fill=guide_legend(title="Property ID")) 
Plot_102

# Bar graph of seeing the saved energy of each period with demand response potential
Plot_103 <- ggplot(flexibility_periods_full_new5.2, aes(x = run_id , y = Shiftable_energy_potential_kWh)) +
  geom_bar(stat = "identity") +
...
Plot_104 <- ggplot(flexibility_periods_full_new5.2, aes(x = (Heat_free_hours))) +
...
Plot_104 <- Plot_104 + guides(fill=guide_legend(title="Property ID")) 
Plot_104
...
Plot_105 <- ggplot(flexibility_periods_full_new5.2, aes(x = (Internal_temperature_decay))) +
...
Plot_105 <- Plot_105 + guides(fill=guide_legend(title="Property ID")) 
Plot_105
