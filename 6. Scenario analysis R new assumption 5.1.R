...
# Date - 09/08/2025
# Description of new script: Implementing assumption 5.1
...
# New assumption 5.1: Internal temperature is greater than 18 degreesC but lesss than 21C as that is for now the threshold
...
filtered_data_new5.1 <- filtered_data %>%
  filter(On_Off_flag == "On" & Hour_Minute >= "17:00:00" & run_length_mins >= 60 | 
           On_Off_flag == "On - Compressor Off" & Hour_Minute >= "17:00:00" | 
           On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 18 & Internal_Air_Temperature <= 21 & External_temp_range <= 2) 
...
filtered_data_on_new5.1 <- filtered_data %>%
...
filtered_data_off_new5.1 <- filtered_data %>%
  filter(On_Off_flag == "Off" & run_length_mins >= 90 & Hour_Minute >= "17:00:00" & Internal_Air_Temperature >= 18 & Internal_Air_Temperature <= 21 & External_temp_range <= 2)

# Filtering for when the dates in the 'On' an 'Off' periods match 
filtered_data_on_new5.1 <- filter(filtered_data_on_new5.1, date %in% filtered_data_off_new5.1$date & Property_ID %in% filtered_data_off_new5.1$Property_ID)
...
filtered_data_new5.1 <- left_join(filtered_data_new5.1,EoH_meta_data, by = "Property_ID")

# Ensuring HP size is formatted as a number 
filtered_data_new5.1$HP_Size_kW <- as.numeric(filtered_data_new5.1$HP_Size_kW)

# Calculating the average hourly consumption across a given period of on or off
filtered_data_new5.1 <- filtered_data_new5.1 %>%
...
filtered_data_new5.1$percent_work_output <- round(filtered_data_new5.1$percent_work_output, digits = 1)
...
filtered_data_new5.1 <- unique(filtered_data_new5.1) 
...
filtered_data_new5.1$date <- ymd(filtered_data_new5.1$date)

# Ensure the above variable is formatted as a number 
filtered_data_new5.1$On_Off_flag_num <- as.numeric(filtered_data_new5.1$On_Off_flag_num)
...
filtered_data_new5.1 <- na.omit(filtered_data_new5.1)
...
#filtered_data_new5.1$consecutive <- filtered_data_new5.1$consecutive - lag(filtered_data_new5.1$consecutive, default = 0)
...
filtered_data_new5.1$Technical_potential_demand_response_kW <- round(filtered_data_new5.1$Technical_potential_demand_response_kW, digits = 2)
filtered_data_new5.1$Shiftable_energy_potential_kWh <- round(filtered_data_new5.1$Shiftable_energy_potential_kWh, digits = 2)
...
filtered_data_new5.1 <- filtered_data_new5.1 %>%
  mutate(Shiftable_hourly_energy_potential_kWh = (Shiftable_energy_potential_kWh / (run_length_mins))*60) %>%
  mutate(half_Shiftable_hourly_energy_potential_kWh = Shiftable_hourly_energy_potential_kWh/2)

# Rounding to an appropiate degree of accuracy and moving before meta data variables
filtered_data_new5.1$Shiftable_hourly_energy_potential_kWh <- round(filtered_data_new5.1$Shiftable_hourly_energy_potential_kWh, digits = 2)
filtered_data_new5.1$half_Shiftable_hourly_energy_potential_kWh <- round(filtered_data_new5.1$half_Shiftable_hourly_energy_potential_kWh, digits = 2)

# Relocating variables so they are before meta data 
filtered_data_new5.1 <- filtered_data_new5.1  %>%  
  relocate(Shiftable_hourly_energy_potential_kWh, .before = Delivery_Contractor) %>%
  relocate(half_Shiftable_hourly_energy_potential_kWh, .before = Delivery_Contractor)
...
filtered_data_new5.1$Shiftable_hourly_energy_potential_kWh <- as.numeric(filtered_data_new5.1$Shiftable_hourly_energy_potential_kWh)
filtered_data_new5.1$total_hourly_consumption_kWh <- as.numeric(filtered_data_new5.1$total_hourly_consumption_kWh)
filtered_data_new5.1$total_hourly_consumption_kWh_lag <- as.numeric(filtered_data_new5.1$total_hourly_consumption_kWh_lag)

table(filtered_data_new5.1$date)

# Creating a variable for the error rate in results 
filtered_data_new5.1 <- filtered_data_new5.1 %>%
  arrange(Property_ID,date) %>%
  group_by(Property_ID,date) %>%
  mutate(error_rate = ((abs(Shiftable_hourly_energy_potential_kWh - (total_hourly_consumption_kWh_lag)))/(total_hourly_consumption_kWh_lag))*100) %>%
  mutate(accuracy = 100 - error_rate) %>%
  ungroup()

# Rounding percentage and error rate to 2.d.p
filtered_data_new5.1$error_rate <- round(filtered_data_new5.1$error_rate, digits = 1)
filtered_data_new5.1$accuracy <- round(filtered_data_new5.1$accuracy, digits = 1)
...
#post_processing_full <- filtered_data_new5.1 %>%
#  arrange(Property_ID,date,day,Hour) %>%
#  group_by(Property_ID,date,day) %>%
#  mutate(error_rate = 
#           ((abs(Shiftable_hourly_energy_potential_kWh - lag(average_hourly_consumption_kWh)))/lag(average_hourly_consumption_kWh))*100) %>%
#  mutate(accuracy = 100 - error_rate) %>%
#  ungroup()

# Filtering for consecutive On-off periods
post_processing_full_new5.1 <- filtered_data_new5.1 %>%
  filter(consecutive_new == 1 | consecutive_new == -1)

# Creating a dataframe for flexibility periods within the dataset
flexibility_periods_new5.1 <- post_processing_full_new5.1 %>%
  filter(On_Off_flag == "Off")

# Converting run_id into a factor to be able to plot  
flexibility_periods_new5.1$run_id <- as.factor(flexibility_periods_new5.1$run_id)

# Filtering for just Off periods to be able to look at accuracy 
flexibility_periods_full_new5.1 <- flexibility_periods_new5.1 %>%
  filter(On_Off_flag == "Off")

# Demand response for each period 
Plot_88 <- ggplot(flexibility_periods_full_new5.1, aes(x = run_id , y = Technical_potential_demand_response_kW)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW), colour = "red", linetype = 'dotted', size = 1) +
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) + 0.3, label = "Mean technical potential for demand response", colour = "red", size = 3) +
  labs(x = "'Off' period" ,
...
Plot_88 <- Plot_88 + guides(fill=guide_legend(title="Property ID")) 
Plot_88

# Move accuracy and error columns so that 
flexibility_periods_full_new5.1 <- flexibility_periods_full_new5.1  %>%  
  relocate(accuracy, .before = Interval_consumption_kWh) %>%
  relocate(error_rate, .before = Interval_consumption_kWh)

# Getting rid of Null values
flexibility_periods_full_new5.1 <- na.omit(flexibility_periods_full_new5.1)
...
median(flexibility_periods_full_new5.1$Measured_accuracy_percent) # median accuracy of results i 98.3%
mean(flexibility_periods_full_new5.1$Measured_accuracy_percent) # mean accuract is 95%
summary(flexibility_periods_full_new5.1$Measured_accuracy_percent)

str(post_processing_full_new5.1)

# Converting run ID into factor to be able to plot 
post_processing_full_new5.1$run_id <- as.factor(post_processing_full_new5.1$run_id)
flexibility_periods_full_new5.1$run_id <- as.factor(flexibility_periods_full_new5.1$run_id)

# Visualising the accuracy
Plot_89 <- ggplot(flexibility_periods_full_new5.1, aes(x = run_id , y = Measured_accuracy_percent)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.1$Measured_accuracy_percent), colour = "red", linetype = 'dotted', size = 1) +
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.1$Measured_accuracy_percent) + 6, label = "Average accuracy of calculated period", colour = "red", size = 3) +
  labs(x = "run ID" ,
       y = "Accuracy (%) ",
...
mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) # 2.06kW  per period
median(flexibility_periods_full_new5.1$half_Shiftable_hourly_energy_potential_kWh) # 0.97 kW per half hour of flexibility
summary(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh) # 5.958 kWh of technical potential per event  
sd(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh) # 3.79 (very large)

# Summary statistics of results 
summary(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh)
sd(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh)

# Calculating optimal number of bins for plotting histogram

# Finding value of n for size of data
n1 <- n_distinct(flexibility_periods_full_new5.1)
print(n1)  #610 data size

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) - min(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW)

Q3 <- quantile(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW, 0.25) 
...
Plot_90 <- ggplot(flexibility_periods_full_new5.1, aes(x = Technical_potential_demand_response_kW)) +
  geom_histogram(bins = Freedman_Diaconic_bin3, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW), colour = "orange", linetype = 'dotted', size = 1)+
  #annotate("label", x =  median(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 60, label = "Median demand response per period (kW)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW), colour = "red", linetype = 'dotted', size = 1)+
  #annotate("label", x =  mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 50, label = "Mean demand response per period (kW)", colour = "red", size = 3) +
...
Plot_90 <- Plot_90 + guides(fill=guide_legend(title="Property ID")) 
Plot_90

# Boxplot of demand response potential 
Plot_91 <- ggplot(flexibility_periods_full_new5.1, aes(y = Technical_potential_demand_response_kW)) +
...
Plot_91 <- Plot_91 + guides(fill=guide_legend(title="Property ID")) 
Plot_91

Panel_plot1 <- Plot_91 + Plot_92
Panel_plot1

# Length of flexibility periods 
summary(flexibility_periods_full_new5.1$Period_length_minutes)/60 # 1.167 hours of flexibility per heat pump
sd((flexibility_periods_full_new5.1$Period_length_minutes)/60)

# Bar graph of seeing the average run length of each flexibility period
Plot_93 <- ggplot(flexibility_periods_full_new5.1, aes(x = run_id , y = (Heat_free_hours))) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.1$Heat_free_hours), colour = "red", linetype = 'dotted', size = 1)+
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.1$Heat_free_hours) + .7 , label = "Average length of period", colour = "red", size = 3) +
  labs(x = "Period" ,
...
Plot_93 <- Plot_93 + guides(fill=guide_legend(title="Property ID")) 
Plot_93

# Bar graph of seeing the saved energy of each period with demand response potential
Plot_94 <- ggplot(flexibility_periods_full_new5.1, aes(x = run_id , y = Shiftable_energy_potential_kWh)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh), colour = "red", linetype = 'dotted', size = 1)+
  annotate("label", x = 300, y = mean(flexibility_periods_full_new5.1$Shiftable_energy_potential_kWh) + .7 , label = "Average saved energy per period", colour = "red", size = 3) +
  labs(x = "Period" ,
...
Plot_94 <- Plot_94 + guides(fill=guide_legend(title="Property ID")) 
Plot_94

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.1$Heat_free_hours)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.1$Heat_free_hours) - min(flexibility_periods_full_new5.1$Heat_free_hours)

Q3 <- quantile(flexibility_periods_full_new5.1$Heat_free_hours, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.1$Heat_free_hours, 0.25) 
...
# Histogram of number of heat free hours
Plot_95 <- ggplot(flexibility_periods_full_new5.1, aes(x = (Heat_free_hours))) +
  geom_histogram(bins = Freedman_Diaconic_bin4, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.1$Heat_free_hours), colour = "orange", linetype = 'dotted', size = 1) +
  #annotate("label", x =  median(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 60, label = "Median length of time (hours)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.1$Heat_free_hours), colour = "red", linetype = 'dotted', size = 1) +
  #annotate("label", x =  mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 50, label = "Mean length of time (hours)", colour = "red", size = 3) +
...
Plot_95 <- Plot_95 + guides(fill=guide_legend(title="Property ID")) 
Plot_95

# Summary of internal temperature decay 
summary(flexibility_periods_full_new5.1$Internal_temperature_decay)
sd(flexibility_periods_full_new5.1$Internal_temperature_decay)

# Freedman-Diaconis rule 
summary(flexibility_periods_full_new5.1$Internal_temperature_decay)

# Finding range in normalized data
range <- max(flexibility_periods_full_new5.1$Internal_temperature_decay) - min(flexibility_periods_full_new5.1$Internal_temperature_decay)

Q3 <- quantile(flexibility_periods_full_new5.1$Internal_temperature_decay, 0.75) 
Q1 <- quantile(flexibility_periods_full_new5.1$Internal_temperature_decay, 0.25) 
...
Plot_96 <- ggplot(flexibility_periods_full_new5.1, aes(x = (Internal_temperature_decay))) +
  geom_histogram(bins = Freedman_Diaconic_bin5, color = "#000000", fill = "#FFFFFF") +
  geom_vline(xintercept = median(flexibility_periods_full_new5.1$Internal_temperature_decay), colour = "orange", linetype = 'dotted', size = 1)+
  #annotate("label", x =  median(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 60, label = "Median length of time (hours)", colour = "orange", size = 3) +
  geom_vline(xintercept = mean(flexibility_periods_full_new5.1$Internal_temperature_decay), colour = "red", linetype = 'dotted', size = 1)+
  #annotate("label", x =  mean(flexibility_periods_full_new5.1$Technical_potential_demand_response_kW) , y = 50, label = "Mean length of time (hours)", colour = "red", size = 3) +
...
Plot_96 <- Plot_96 + guides(fill=guide_legend(title="Property ID")) 
Plot_96